...
接下来先把 `upstream`、本地仓库和 `origin` 这三个角色一次讲清楚，再看整条贡献流程怎么展开。
...
- `origin` 解决“当前这份改动先推到哪里”
- `upstream` 解决“当前这条主线应该跟谁同步”
...
```text
[1] upstream ---fork---> origin
    先在平台上复制一份自己的仓库副本

[2] origin ---clone---> 本地仓库
    把自己的 fork 拉到本地开始工作

[3] 本地仓库 ---add remote upstream---> upstream
    给本地补上“真正主仓库”的入口

[4] upstream/main ---fetch / merge --ff-only---> 本地 main
    先把开发基线同步到上游最新

[5] 本地 main ---switch -c---> 本地 feature/*
    从同步后的主线切出功能分支

[6] 本地 feature/* ---push---> origin/feature/*
    先把改动推到自己的 fork

[7] origin/feature/* ---Pull Request---> upstream/main
    由上游维护者审核并决定是否合入
```
...
执行 `git push -u origin feature/fix-typo` 后：

+--------------------------------------+
| 个人远端 origin                      |
| origin/main -> C                     |
| origin/feature/fix-typo -> D         |
+--------------------------------------+

接下来，再由 `origin/feature/fix-typo` 发起 PR，目标是 `upstream/main`。
```
...
```text
+--------------------------------------+
| 第1步：git push origin feature/...   |
| 改动先到达自己的 fork                |
+--------------------------------------+
                  |
                  v
+--------------------------------------+
| 第2步：Open Pull Request             |
| 上游开始审核这份候选贡献             |
+--------------------------------------+
                  |
                  v
